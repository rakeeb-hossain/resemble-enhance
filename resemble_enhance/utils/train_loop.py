...
class GenFeeder(Protocol):
    def __call__(self, engine: Engine,
                 batch: dict[str, Tensor]) -> tuple[Tensor, dict[str, Tensor]]: ...


class DisFeeder(Protocol):
    def __call__(self, engine: Engine,
                 batch: Optional[dict[str, Tensor]], fake: Tensor) -> dict[str, Tensor]: ...


class TrainLoop:
    def __init__(self, run_dir=None, train_dl=None, load_G=None, feed_G=None, load_D=None, feed_D=None, update_every=5_000, eval_every=5_000,
        backup_steps=(5_000, 100_000, 500_00), device="cuda", eval_fn=None, gan_training_start_step=None):
        if None in [run_dir, train_dl, load_G, feed_G]:
            raise TypeError("necessary fields missing")
        ...
        self.device = device
        self.gan_training_start_step = gan_training_start_step
    ...
    @property
    def eval_dir(self) -> Optional[Path]:
    ...
    @classmethod
    def get_running_loop(cls) -> Optional["TrainLoop"]:
    ...
    @classmethod
    def get_running_loop_global_step(cls) -> Optional[int]:
        if loop := cls.get_running_loop():
            return loop.global_step
        return None

    @classmethod
    def get_running_loop_viz_path(cls, name: str, suffix: str) -> Optional[Path]:
        if loop := cls.get_running_loop():
